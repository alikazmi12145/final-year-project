...
class UrduOCR:
    ...
    def __init__(self, tesseract_cmd=None):
        ...
        if tesseract_cmd:
            ...
            if found:
                ...
                # leave unset ΓÇö we will surface an error when attempting OCR
        ...
        if self.pytesseract:
            try:
                ...
                logger.info("Γ£à Tesseract is available and responding.")
    ...
    def _set_tesseract_cmd(self, path):
        ...
        if self.pytesseract:
            ...
            logger.info(f"Γ£à Tesseract command set to: {path}")
    ...
    def extract_text(self, image_path, languages=None):
        ...
        logger.info("=" * 60)
        logger.info("≡ƒÜÇ STARTING 5-STRATEGY OCR APPROACH (includes artistic text)")
        logger.info(f"≡ƒôï Strategies: adaptive_binary, inverted, preprocessed, light_preprocess, original_grayscale")
        logger.info(f"≡ƒô¥ Languages requested: {languages}")
        logger.info("=" * 60)
        ...
        logger.info(f"≡ƒÄ» Using language: {lang_str} (Urdu ONLY to avoid English interference)")
        ...
        for strategy_name, processed_img in strategies:
            logger.info(f"≡ƒöä Trying strategy: {strategy_name}")
            
            for lang in lang_options:
                for psm in psm_modes:
                    # Use Urdu language only, without character whitelist (too restrictive)
                    custom_config = f"--oem 3 --psm {psm}"
                    logger.debug(f"  ≡ƒöì Trying lang={lang}, PSM {psm}...")
                    try:
                        ...
                        if score > best_conf or (score == best_conf and len(cleaned_text) > len(best_text)):
                            ...
                            best_lang = lang
                            logger.debug(f"  Γ£à New best: score={score:.2f}, quality={quality_score:.2f}")
            ...
            if best_text and best_conf > 70 and len(best_text) > 10:
                logger.info(f"Γ£à Good result with {best_strategy}, skipping remaining strategies")
                break

        # Log final result
        if best_text:
            logger.info(f"≡ƒÄ» Final OCR result ({best_strategy}, lang={best_lang}): '{best_text}' ({len(best_text)} chars, {best_conf:.1f}% confidence, PSM {best_psm})")
        else:
            logger.warning(f"ΓÜá∩╕Å No text extracted after trying all strategies and PSM modes")
        ...
        if best_text:
            ...
            if quality_score < 0.3:
                logger.warning(f"ΓÜá∩╕Å Text quality is poor (score: {quality_score:.2f})")
        ...
        logger.info(f"Γ£à OCR completed successfully (quality: {quality_score:.2f})")
    ...
    def _preprocess_image(self, image):
        ...
        logger.info("≡ƒû╝∩╕Å Starting GENTLE preprocessing (optimized for Urdu)")
        ...
        if width < min_w:
            ...
            logger.info(f"≡ƒôÉ Resized image from {original_size} to {new_size}")
        elif width > max_w:
            scale = max_w / width
            new_size = (int(width * scale), int(height * scale))
            image = image.resize(new_size, Image.Resampling.LANCZOS)
            logger.info(f"≡ƒôÉ Downscaled large image from {original_size} to {new_size}")
        ...
        logger.info("Γ£à Applied gentle contrast (1.2x)")
        
        # VERY GENTLE sharpening (just 1.1x)
        image = ImageEnhance.Sharpness(image).enhance(1.1)
        logger.info("Γ£à Applied gentle sharpening (1.1x)")
        
        logger.info("Γ£à GENTLE preprocessing completed - avoided over-processing")
        return image
    ...
    def _clean_extracted_text(self, text):
        ...
        for char in text:
            if (is_urdu_char(char) or           # Urdu/Arabic characters
                ...
                char in '█ö╪î╪¢╪ƒ'):                 # Urdu punctuation
                cleaned.append(char)
    ...
    def _binarize_adaptive(self, image):
        ...
        """
        logger.info("≡ƒû╝∩╕Å ADAPTIVE BINARIZATION - for artistic text")
        ...
        if width < min_w:
            ...
            new_size = (int(width * scale), int(height * scale))
            image = image.resize(new_size, Image.Resampling.LANCZOS)
            logger.info(f"≡ƒôÉ Resized to {new_size}")
        ...
        if has_cv2:
            ...
            if black_pixels > white_pixels:
                ...
                logger.info("≡ƒöä Detected inverted text - flipped image")
            
            # Convert back to PIL
            result = Image.fromarray(binary)
            logger.info("Γ£à Adaptive binarization complete")
            return result
        else:
    ...
    def _invert_colors(self, image):
        ...
        """
        logger.info("≡ƒû╝∩╕Å INVERTING colors - for light text on dark background")
        ...
        logger.info("Γ£à Color inversion complete")
        return inverted
    ...
    def _light_preprocess(self, image):
        ...
        """
        logger.info("≡ƒû╝∩╕Å MINIMAL preprocessing - just grayscale")
        ...
        if width < min_w:
            ...
            new_size = (int(width * scale), int(height * scale))
            image = image.resize(new_size, Image.Resampling.LANCZOS)
            logger.info(f"≡ƒôÉ Downscaled to {new_size}")
        
        # Convert to grayscale - that's it!
        image = image.convert("L")
        logger.info("Γ£à MINIMAL preprocessing done")
    ...
    def detect_language_from_image(self, image_path):
        ...
        try:
            ...
            if 'Arabic' in osd or 'Urdu' in osd:
                logger.info("≡ƒîì Detected: Urdu/Arabic script")
                return 'urd'
            elif 'Latin' in osd or 'English' in osd:
                logger.info("≡ƒîì Detected: English/Latin script")
                return 'eng'
        ...
        try:
            ...
            if urdu_count > eng_count:
                logger.info(f"≡ƒîì Detected: Urdu (Urdu chars: {urdu_count}, Eng: {eng_count})")
                return 'urd'
            elif eng_count > urdu_count:
                logger.info(f"≡ƒîì Detected: English (Eng chars: {eng_count}, Urdu: {urdu_count})")
                return 'eng'
            else:
                logger.info(f"≡ƒîì Detected: Mixed (Urdu: {urdu_count}, Eng: {eng_count})")
                return 'mixed'
    ...
    def extract_from_url(self, image_url):
        ...
        try:
            ...
            logger.error(f"Γ¥î Error downloading or opening image from URL: {e}")
