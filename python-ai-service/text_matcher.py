...
class TextMatcher:
    ...
    def __init__(self):
        if not HAS_RAPIDFUZZ:
            logger.error("Γ¥î rapidfuzz not installed! Install with: pip install rapidfuzz")
            raise ImportError(
                "rapidfuzz is required for text matching. "
                "Install it with: pip install rapidfuzz"
            )
        else:
            logger.info("Γ£à Text matcher initialized with rapidfuzz")
    ...
    def rank_matches(self, extracted_text: str, rekhta_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(f"≡ƒôè Ranked {len(scored_results)} matches by similarity")
        if scored_results:
            logger.info(f"≡ƒÅå Best match score: {scored_results[0]['match_score']:.1f}%")
    ...
    def find_best_match(self, extracted_text: str, rekhta_results: List[Dict[str, Any]], 
        ...
        if score >= min_score:
            logger.info(f"Γ£à Found good match: {best.get('title', 'Unknown')} (score: {score:.1f}%)")
            return best, score
        else:
            logger.info(f"ΓÜá∩╕Å Best match score {score:.1f}% below threshold {min_score}%")
            return None, 0.0
