...
logger.info("≡ƒÜÇ Initializing OCR engine...")
ocr_engine = get_ocr_engine()
logger.info("Γ£à OCR engine initialized")
...
def analyze_image():
    ...
    try:
        ...
        logger.info(f"≡ƒô╕ Processing image: {filename}")
        ...
        if result['success']:
            ...
            logger.error(f"Γ¥î OCR Failed: {result.get('error', 'Unknown error')}")
        ...
        logger.error(f"Γ¥î Image analysis error: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("≡ƒÉì Urdu Poetry OCR Service (Lightweight)")
    print("=" * 70)
    print(f"≡ƒÜÇ Starting Flask server on http://localhost:{port}")
    print("≡ƒñû OCR Engine: Tesseract (Urdu/English/Arabic)")
    print("")
    print("≡ƒôí Endpoints:")
    print("   - GET  /health")
