...
try:
    ...
    print(f"ΓÜá∩╕Å  ML features not available: {e}")
...
if ML_FEATURES_AVAILABLE:
    logger.info("≡ƒÜÇ Initializing AI engines...")
    try:
        semantic_engine = get_semantic_engine()
        ocr_engine = get_ocr_engine()
        rekhta_service = get_rekhta_service()
        text_matcher = get_text_matcher()
        logger.info("Γ£à AI engines initialized")
    except Exception as e:
        logger.warning(f"ΓÜá∩╕Å  Could not initialize ML engines: {e}")
        ML_FEATURES_AVAILABLE = False
else:
    logger.info("ΓÜí Running in lightweight mode (Qaafia + Harf-Ravi only)")
...
def analyze_image_with_rekhta():
    ...
    try:
        ...
        logger.info(f"≡ƒô╕ Processing image with Rekhta integration: {filename}")
        
        try:
            # Step 1: Detect language if enabled
            detected_lang = None
            if auto_detect:
                logger.info("≡ƒöì Auto-detecting language...")
                detected_lang = ocr_engine.detect_language_from_image(filepath)
                logger.info(f"Γ£à Detected language: {detected_lang}")
            
            # Step 2: Extract text with OCR
            logger.info("≡ƒô¥ Extracting text with OCR...")
            ...
            logger.info(f"Γ£à Extracted text: {extracted_text[:100]}...")
            
            # Step 3: Search Rekhta
            logger.info("≡ƒöì Searching Rekhta...")
            ...
            if rekhta_result.get('success') and rekhta_result.get('matches'):
                logger.info("≡ƒôè Ranking matches by similarity...")
                ...
                logger.info(f"Γ£à Found {len(ranked_matches)} ranked matches")
                if best_match:
                    logger.info(f"≡ƒÅå Best match: {best_match.get('title')} by {best_match.get('poet')} ({best_score:.1f}%)")
        ...
        logger.error(f"Γ¥î Image + Rekhta analysis error: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def semantic_search():
    ...
    try:
        ...
        logger.info(f"≡ƒöì Semantic search: '{query}' (top_k={top_k}, threshold={threshold})")
        ...
        logger.error(f"Γ¥î Semantic search error: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def create_index():
    ...
    try:
        ...
        logger.info(f"≡ƒôÜ Creating semantic index for {len(poems)} poems...")
        ...
        logger.error(f"Γ¥î Index creation error: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def update_index():
    ...
    try:
        ...
        logger.info(f"≡ƒôÜ Updating semantic index with {len(new_poems)} new poems...")
        ...
        logger.error(f"Γ¥î Index update error: {str(e)}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def ai_qaafia_search():
    ...
    try:
        ...
        logger.info(f"≡ƒöì AI Qaafia search for: '{word}'")
        ...
        for candidate in all_words:
            if candidate not in seen_words:
                ...
                if similarity >= min_similarity:
                    ...
                    if similarity >= 0.85:
                        match_type = 'exact'  # ╪¿┘ä┌⌐┘ä ╪»╪▒╪│╪¬
                    elif similarity >= 0.65:
                        match_type = 'similar'  # ┘à┘à╪º╪½┘ä
                    elif similarity >= 0.50:
                        match_type = 'close'  # ┘é╪▒█î╪¿
                    else:
                        match_type = 'loose'  # ┌ê┌╛█î┘ä╪º
        ...
        logger.info(f"Γ£à Found {len(rhymes_found)} rhymes for '{word}' (min_similarity={min_similarity})")
        ...
        logger.error(f"Γ¥î AI Qaafia search error: {str(e)}")
...
def ai_harf_ravi_extract():
    ...
    try:
        ...
        logger.info(f"≡ƒöì AI Harf-e-Ravi extraction from: '{text[:50]}...'")
        ...
        logger.info(f"Γ£à Extracted Harf-Ravi: {harf_ravi} (confidence: {result['analysis']['confidence']})")
        
        return jsonify(result), 200
        
    except Exception as e:
        logger.error(f"Γ¥î AI Harf-e-Ravi extraction error: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print("≡ƒÉì AI Multimodal Search Python Service with ML Models")
    print("=" * 70)
    print(f"≡ƒÜÇ Starting Flask server on http://localhost:{port}")
    print("≡ƒñû ML Models Loaded:")
    print("   Γ£ô Semantic Search: paraphrase-multilingual-mpnet-base-v2")
    print("   Γ£ô OCR Engine: Tesseract (Urdu/English/Hindi)")
    print("   Γ£ô Voice Recognition: Google Speech API")
    print("   Γ£ô Rekhta Integration: Poetry matching & search")
    print("   Γ£ô Fuzzy Matching: RapidFuzz for text comparison")
    print("   Γ£ô Qaafia Service: Urdu rhyming patterns (lightweight)")
    print("   Γ£ô Harf-e-Ravi: Core rhyme extraction")
    print("")
    print("≡ƒôí Available endpoints:")
    print("   - GET  /health")
    ...
    print("   - POST /ai/qaafia-search            (≡ƒÄ» AI Rhyming word search)")
    print("   - POST /ai/harf-ravi-extract        (≡ƒÄ» AI Harf-e-Ravi extraction)")
    print("=" * 70)
    
    # Try to load existing index
    try:
        if semantic_engine.load_index():
            print(f"Γ£à Loaded existing semantic index: {semantic_engine.index.ntotal} poems")
        else:
            print("ΓÜá∩╕Å  No existing index found. Create one via /index/create")
    except Exception as e:
        print(f"ΓÜá∩╕Å  Could not load index: {str(e)}")
    
    print("=" * 70)
    print("Γ£¿ All Python AI services ready! Use 'python app.py' to run.")
    print("=" * 70)
