...
class SemanticSearchEngine:
    ...
    def __init__(self, model_name='paraphrase-multilingual-mpnet-base-v2'):
        ...
        """
        logger.info(f"≡ƒöä Loading semantic search model: {model_name}")
        self.model = SentenceTransformer(model_name)
        self.index = None
        self.poems_data = []
        self.dimension = 768  # mpnet-base embedding dimension
        
        logger.info("Γ£à Semantic search model loaded successfully")
    ...
    def create_index(self, poems):
        ...
        try:
            if not poems or len(poems) == 0:
                logger.warning("ΓÜá∩╕Å No poems provided for indexing")
                return False
            
            logger.info(f"≡ƒöä Creating semantic index for {len(poems)} poems")
            ...
            # Generate embeddings
            logger.info("≡ƒöä Generating embeddings...")
            ...
            logger.info(f"Γ£à Semantic index created with {self.index.ntotal} poems")
            return True
            
        except Exception as e:
            logger.error(f"Γ¥î Error creating semantic index: {str(e)}")
            return False
    ...
    def search(self, query, top_k=20, threshold=0.3):
        ...
        try:
            if self.index is None or self.index.ntotal == 0:
                logger.warning("ΓÜá∩╕Å No semantic index available")
                return []
            ...
            logger.info(f"Γ£à Found {len(results)} semantic matches for query: {query[:50]}")
            return results
    ...
    def save_index(self, filepath='semantic_index.pkl'):
        ...
        try:
            if self.index is None:
                logger.warning("ΓÜá∩╕Å No index to save")
                return False
            ...
            logger.info(f"Γ£à Semantic index saved to {filepath}")
            return True
            
        except Exception as e:
            logger.error(f"Γ¥î Error saving index: {str(e)}")
            return False
    ...
    def load_index(self, filepath='semantic_index.pkl'):
        ...
        try:
            ...
            if not os.path.exists(faiss_path) or not os.path.exists(filepath):
                logger.warning(f"ΓÜá∩╕Å Index files not found: {filepath}")
                return False
            ...
            logger.info(f"Γ£à Semantic index loaded: {self.index.ntotal} poems")
            return True
            
        except Exception as e:
            logger.error(f"Γ¥î Error loading index: {str(e)}")
            return False
    ...
    def update_index(self, new_poems):
        ...
        try:
            ...
            logger.info(f"≡ƒöä Updating index with {len(new_poems)} new poems")
            ...
            logger.info(f"Γ£à Index updated: now {self.index.ntotal} poems")
            return True
            
        except Exception as e:
            logger.error(f"Γ¥î Error updating index: {str(e)}")
            return False
